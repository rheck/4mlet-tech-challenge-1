...
def parse(site_option, site_suboption, year):
    ...
    This parser reads the country, quantity, and value columns from the table.
    ...
    for row in table.tbody.find_all('tr'):
        ...
        parsed_content.append({
            ...
            'value': table_utils.get_column_int(columns[2]),
            'value_text': columns[2].text.strip()
        })

    total_columns = table.tfoot.find('tr').find_all('td')
        
    return parsed_content, table_utils.get_column_int(total_columns[1]), total_columns[1].text.strip(), table_utils.get_column_int(total_columns[2]), total_columns[2].text.strip()
