...
def route():
    ...
    try:
        ...
        parsed_content, totals, totals_text, value, value_text = countries_parser.parse(config.site_imported_option, suboption, year)
        
        country_service.persist_parsed_content(parsed_content, totals_text, value_text, site_suboption, year, "imported")
    except:
        try:
            real_time = False
            success, parsed_content, totals, totals_text, value, value_text, parse_date = country_service.load_parsed_content(site_suboption, year, "imported")
